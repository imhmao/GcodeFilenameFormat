...
class GcodeFilenameFormat(OutputDevice, Extension):
    def __init__(self):
        ...
        Application.getInstance().getPreferences().addPreference(
            "gcode_filename_format/last_used_type", "")
        Application.getInstance().getPreferences().addPreference(
            "gcode_filename_format/dialog_save_path", "")
        Application.getInstance().getPreferences().addPreference(
            "gcode_filename_format/filename_format", DEFAULT_FILENAME_FORMAT)

        self.setName(catalog.i18nc("@item:inmenu", "Gcode Filename Format"))
        self.setShortDescription(catalog.i18nc(
            "@action:button Preceded by 'Ready to'.", "Save Gcode"))
    ...
    def requestWrite(self, nodes, file_name=None, limit_mimetypes=None, file_handler=None, **kwargs):
        ...
        if "preferred_mimetypes" in kwargs and kwargs["preferred_mimetypes"] is not None:
            ...
            preferred_mimetypes = Application.getInstance().getPreferences().getValue(
                "gcode_filename_format/last_used_type")
        ...
        if len(file_types) == 0:
            ...
            raise OutputDeviceError.WriteRequestFailedError(catalog.i18nc(
                "@info:warning", "There are no file types available to write with!"))
        ...
        for item in file_types:
            ...
            if preferred_mimetype == item["mime_type"]:
                ...
                file_name = self.parseFilenameFormat(
                    filename_format, file_name, application, global_stack)
        ...
        # 如果 self._dos == true and 扩展名是.gcode，则改为8.3结构的 .g
        if self._dos and os.path.splitext(file_name)[1] == ".gcode":
            fn = os.path.splitext(os.path.basename(file_name))
            fn = (fn[0][0:8] + ".g").replace(" ", "")
    ...
    def parseFilenameFormat(self, filename_format, file_name, application, global_stack):
        ...
        print_time = print_information.currentPrintTime.getDisplayString(
        ...
        # TODO: split on brackets only
        tokens = re.split(r'\W+', filename_format)
        ...
        return filename_format.strip().lstrip("\\/").upper()
    ...
    def _createDialogue(self):
        ...
        ).getPluginPath(self.getPluginId()), "Format.qml")
    ...
    def _createHelpDialog(self):
        qml_file_path = os.path.join(PluginRegistry.getInstance(
        ).getPluginPath(self.getPluginId()), "Help.qml")
        component = Application.getInstance().createQmlComponent(qml_file_path)
    ...
    def getModifiedPrintSettings(self, application, global_stack):
        slice_info = application._plugin_registry.getPluginObject(
            "SliceInfoPlugin")
        modified_print_settings = slice_info._getUserModifiedSettingKeys()

        machine_id = global_stack.definition.getId()
        manufacturer = global_stack.definition.getMetaDataEntry(
            "manufacturer", "")
